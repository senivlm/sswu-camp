        ...
        public static void Main(string[] args)
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    else if (operation?.ToUpper() == Commands.GET_COMMON_PRODUCTS.ToString())
                    {
                        List<Product> commonList = new List<Product>();
                        Storage rhsStorage = new Storage();
                        rhsStorage.AddProductsFromFile(GetPath());
                        Storage.GetCommonProducts(storage, rhsStorage, out commonList);
                        Storage.PrintProducts(commonList);
                    }
                    else if (operation?.ToUpper() == Commands.GET_UNIQUE_PRODUCTS.ToString())
                    {
                        List<Product> uniqueList = new List<Product>();
                        Storage rhsStorage = new Storage();
                        rhsStorage.AddProductsFromFile(GetPath());
                        Storage.GetUniqueProducts(storage, rhsStorage, out uniqueList);
                        Storage.PrintProducts(uniqueList);
                    }
                    else if (operation?.ToUpper() == Commands.GET_COMBINED_LIST.ToString())
                    {
                        List<Product> combinedList = new List<Product>();
                        Storage rhsStorage = new Storage();
                        rhsStorage.AddProductsFromFile(GetPath());
                        Storage.GetCombinedList(storage, rhsStorage, out combinedList);
                        Storage.PrintProducts(combinedList);
                    }
